...
program main
  use mt95, only: genrand_init, genrand_real2, genrand_int31
  implicit none
  !declaration
  real(kind=8) :: trial
  !constants
  integer, parameter :: MAXMOL = 100000
...
99   format(a5)
  ...
  do target=1,no
     ...
     !Note it is different from the expression in slide-and-match.F90
     write(STDOUT,*) target, xo(target), yo(target), zo(target), score !, width
  enddo
end program main
